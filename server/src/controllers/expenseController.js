  ...
  try {
    ...
    if (!groupId && personId) {
      ...
      if (otherUser._id.toString() === userId.toString()) {
        return res
          .status(400)
          .json({ message: "You cannot add an expense with yourself" });
      }
      ...
      if (payers.length === 0) {
        return res
          .status(400)
          .json({ message: "At least one payer with amount > 0 is required" });
      }

      const totalAmount =
        Math.round(payers.reduce((sum, p) => sum + Number(p.amount), 0) * 100) /
        100;
      ...
      if (splitAmong && Array.isArray(splitAmong) && splitAmong.length > 0) {
        debtorIds = splitAmong.filter((id) =>
      ...
      if (splitType === "equally") {
        ...
        if (!percentages) {
          return res
            .status(400)
            .json({ message: "percentages object is required" });
        }
        let totalPct = 0;
        debtorIds.forEach((id) => {
        ...
        if (!exactAmounts) {
          return res
            .status(400)
            .json({ message: "exactAmounts object is required" });
        }
        let totalExact = 0;
        debtorIds.forEach((id) => {
      ...
      const expense = await Expense.create({
        description,
        amount: totalAmount,
        date: req.body.date ? new Date(req.body.date) : new Date(), // ✅ ADD THIS
      ...
      if (!isSelfExpense) {
        for (const debtor of debtorIds) {
          ...
          for (const payer of payers) {
            ...
            const owedToPayer =
              Math.round(totalOwed * payerFraction * 100) / 100;
    ...
    if (!groupId) {
      return res
        .status(400)
        .json({ message: "groupId and description are required" });
    }
    ...
    if (splitAmong && Array.isArray(splitAmong) && splitAmong.length > 0) {
      debtorIds = splitAmong.filter((id) =>
        groupMemberIds.includes(id.toString()),
      );
    }
...
export const getExpenses = async (req, res) => {
  try {
    ...
    const userGroups = await Group.find({ "members.user": userId }).select(
      "_id",
    );
    ...
    if (groupId) {
      ...
      if (!isGroupAdmin && !isCreator) {
        return res.status(403).json({
          message:
            "You can only delete expenses you created. Group admins can delete any expense.",
        });
      }
        ...
        for (const payer of storedPayers) {
          ...
          if (owedToPayer > 0) {
            ...
            if (bal) {
              bal.amount = Math.max(
                0,
                Math.round((bal.amount - owedToPayer) * 100) / 100,
              );
  ...
  try {
    ...
    for (const debtor of oldDebtors) {
      ...
      for (const payer of oldPayers) {
        ...
        if (owed > 0 && groupId) {
          await reverseBalance(payer.memberId, debtor, owed, groupId);
        }
      }
    }
